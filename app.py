...
app = Flask(__name__)
app.secret_key = 'une-cle-secrete-temporaire'

# Récupération des variables d'environnement (nécessaire sur Render)
...
if not SUPABASE_URL:
    raise Exception("SUPABASE_URL non définie")
if not SUPABASE_ANON_KEY:
    raise Exception("SUPABASE_ANON_KEY non définie")
if not SUPABASE_SERVICE_KEY:
    raise Exception("SUPABASE_SERVICE_KEY non définie")
    ...
    if request.method == 'POST':
        ...
        if 'photo' in request.files:
            ...
            if file and file.filename:
                ...
                supabase_admin.storage.from_('photos_candidats').upload(filename, file_bytes, {'content-type': file.mimetype or 'image/jpeg'})
